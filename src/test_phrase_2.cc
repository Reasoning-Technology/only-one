...
int main(int argc ,char **argv){
  ...
  string input_filename = string("test_phrase_2_in.txt");
  ifstream is(input_filename.c_str());
  int fd0;

  string output_filename("test_phrase_2_out.txt");
  ...
  /* input and output headers should be different because the top controlled data is currently input only 
    if( !same(fd0,fd1)){
    errors++;
